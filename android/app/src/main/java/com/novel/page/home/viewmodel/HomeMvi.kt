...
sealed class HomeIntent : MviIntent {
    ...
    // === 数据加载成功的Intent ===
    /** 分类筛选器加载成功 */
    data class CategoryFiltersLoadSuccess(val filters: List<CategoryInfo>) : HomeIntent()
    ...
    data class CategoryRecommendBooksLoadSuccess(
        val books: List<SearchService.BookInfo>,
    ...
    /** 榜单书籍加载成功 */
    data class RankBooksLoadSuccess(val books: List<BookService.BookRank>) : HomeIntent()
    
    /** 榜单书籍加载失败 */
    data class RankBooksLoadFailure(val error: String) : HomeIntent()
    
    /** 分类数据加载成功 */
    data class CategoriesLoadSuccess(val categories: List<HomeCategoryEntity>) : HomeIntent()
            ...
            is HomeIntent.SelectCategoryFilter -> {
                ReduceResult(
                    newState = currentState.copy(
                        ...
                        recommendBooks = if (intent.categoryName == "推荐") emptyList() else currentState.recommendBooks,
                        recommendPage = 1
                    )
                )
            }
                ...
                ReduceResult(
                    newState = currentState.copy(
                        ...
                        homeRecommendBooks = if (intent.isRefresh) {
                            ...
                            currentState.homeRecommendBooks + intent.books
                        },
                ...
                ReduceResult(
                    newState = currentState.copy(
                        ...
                        recommendBooks = if (intent.isLoadMore) {
                            currentState.recommendBooks + intent.books
                        } else {
                            intent.books
                        },
