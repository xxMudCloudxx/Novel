...
class ReadingProgressRepository @Inject constructor(
    ...
    suspend fun saveReadingProgress(progress: ReadingProgressData) = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "保存阅读进度: bookId=${progress.bookId}, chapter=${progress.chapterId}, page=${progress.pageIndex}")
    ...
    suspend fun getReadingProgress(bookId: String): ReadingProgressData? = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d(TAG, "获取阅读进度: bookId=$bookId, found=${progress != null}")
            progress
        } catch (e: Exception) {
            Log.e(TAG, "获取阅读进度失败: bookId=$bookId", e)
            null
        }
    }
    ...
    suspend fun saveReaderSettings(settings: ReaderSettingsData) = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "保存阅读器设置: 字体${settings.fontSize}sp, 翻页效果${settings.pageFlipEffect}")
            val json = gson.toJson(settings)
            userDefaults.setString(READER_SETTINGS_KEY, json)
            Log.d(TAG, "阅读器设置保存成功")
        } catch (e: Exception) {
            Log.e(TAG, "保存阅读器设置失败", e)
        }
    }
    ...
    suspend fun clearReadingProgress(bookId: String) = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "清理阅读进度: bookId=$bookId")
            val progressMap = getAllReadingProgress().toMutableMap()
            ...
            if (removed != null) {
                ...
                Log.d(TAG, "阅读进度不存在，无需清理")
            }
        } catch (e: Exception) {
            Log.e(TAG, "清理阅读进度失败: bookId=$bookId", e)
        }
    }
    ...
    suspend fun clearAllReadingProgress() = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d(TAG, "所有阅读进度清理完成")
        } catch (e: Exception) {
            Log.e(TAG, "清理所有阅读进度失败", e)
        }
    }
    ...
    suspend fun updatePageFlipEffect(bookId: String, pageFlipEffect: PageFlipEffect) = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "更新翻页效果: bookId=$bookId, effect=$pageFlipEffect")
            val progressMap = getAllReadingProgress().toMutableMap()
            ...
            if (existingProgress != null) {
                ...
                Log.d(TAG, "书籍进度不存在，跳过翻页效果更新")
            }
        } catch (e: Exception) {
            Log.e(TAG, "更新翻页效果失败: bookId=$bookId", e)
        }
    }
    ...
    suspend fun updateAllBooksPageFlipEffect(pageFlipEffect: PageFlipEffect) = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "批量更新翻页效果: effect=$pageFlipEffect")
            val progressMap = getAllReadingProgress().toMutableMap()
            var updateCount = 0
            ...
            if (updateCount > 0) {
                val json = gson.toJson(progressMap)
                userDefaults.setString(READING_PROGRESS_KEY, json)
                Log.d(TAG, "批量更新完成，影响$updateCount 本书籍")
            } else {
                Log.d(TAG, "无阅读进度数据，跳过批量更新")
            }
        } catch (e: Exception) {
            Log.e(TAG, "批量更新翻页效果失败", e)
        }
    }
} 